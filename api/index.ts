...
async function getWeatherData(): Promise<WeatherData[]> {
  ...
  const items = await redis.xrevrange(
    "weather-station:0",
    "+",
    "-",
    "COUNT",
...
async function postWeatherData(weatherRecord: WeatherData): Promise<boolean> {
  try {
    ...
    redis.xadd(
      ...
      "humid_prcnt",
      `${weatherRecord.humid_prcnt}`,
      "pressure_kpa",
      `${weatherRecord.pressure_kpa}`,
      "gas_kohms",
      `${weatherRecord.gas_kohms}`,
      "rain_an",
      `${weatherRecord.rain_an}`,
      "light_an",
      `${weatherRecord.light_an}`
    );
  } catch (error) {
    return false;
  }
...
function isJsonWeatherRecord(json: object): boolean {
  return (
    json.hasOwnProperty("timestamp_unix_ms") &&
...
function isWeatherValid(weather: WeatherData): boolean {

  // Sanity checks

  if (
    weather.temp_c == "0" &&
    weather.humid_prcnt == "0" &&
    weather.pressure_kpa == "0" &&
    weather.gas_kohms == "0" &&
    weather.rain_an == "0" &&
    weather.light_an == "0"
  ) {
...
app.post("/api/envdata", async (req, res) => {
  ...
  let validRecord = isWeatherValid(data);
  let reason = "OK";

  if (isWeatherRecord && validRecord) {
